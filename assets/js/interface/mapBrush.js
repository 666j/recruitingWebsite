...
(function(){
	/************初始化echarts图表***********************/
	//geoCoordMap为地名和经纬度 data是地名和需求值的数组
  ...
  /************处理传入的data和Map*********************/
	...
	function initChart(convertedData){
		...
		var mapBrushChart = echarts.init(mapBrushDom);
//		/*******************测试的时候用***********/
//		var convertedData = [
...
/************************刷取的事件处理************************************/
	...
	function renderBrushed(params) {
	    ...
	    getBrush({"data_info":categoryData});
		this.setOption({
			 tooltip : {
		        trigger: 'axis',
...
//		handleData(datas);
		...
		function handleData(data){
			...
			for(var i =0;i<data.length;i++){
				...
				wages.push(data[i]["wage"]);
				rents.push(data[i]['rent']);
				incomes.push(data[i]["wage"]+data[i]['rent']);
			}
...
//		handleData(data);
		function handleData(data){
			...
			if(data[0].status==0){
				...
				salaries =[];
				for(var i =0;i<data.length;i++){
					works.push(data[i]['works']);
					salaries.push(data[i]['wage']);
				}
			}
